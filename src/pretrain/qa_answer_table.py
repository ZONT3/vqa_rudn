...
def load_lxmert_qa(path, model, label2ans):
    """
    Загрузить веса нейронной сети из файла.
    ...
    :param path: Пусть к файлу весов сети
    :param model: Модель LXRT
    :param label2ans: Словарь из ответов с их номерами (как ключи) из fine-tuned QA датасета, в виде:
        {0: 'cat', 1: 'dog', ...}
    :return:
    """
    print("Load QA pre-trained LXMERT from %s " % path)
    loaded_state_dict = torch.load("%s_LXRT.pth" % path)  # Загружает состояние нейронной сети из файла, сохраненное torch.save (у нас pre-trained модель)
